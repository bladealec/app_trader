...
(WITH both_stores AS 	(SELECT name,  
									...
									CASE 	WHEN as_price IS NULL THEN ps_price
						 			...
						 			CASE	WHEN CONCAT(as_genre, ps_genre) ILIKE '%games%' OR CONCAT(as_genre, ps_genre) ILIKE '%entertainment%' OR CONCAT(as_genre, ps_genre) ILIKE '%casino%' OR CONCAT(as_genre, ps_genre) ILIKE '%racing%' OR CONCAT(as_genre, ps_genre) ILIKE '%card%' OR CONCAT(as_genre, ps_genre) ILIKE '%board%' OR name ILIKE '%game%' OR CONCAT(as_genre, ps_genre) ILIKE '%strategy%' OR CONCAT(as_genre, ps_genre) ILIKE '%puzzle%' OR CONCAT(as_genre, ps_genre) ILIKE '%action%' OR CONCAT(as_genre, ps_genre) ILIKE '%comics%' OR CONCAT(as_genre, ps_genre) ILIKE '%role playing%' OR CONCAT(as_genre, ps_genre) ILIKE '%arcade%' OR CONCAT(as_genre, ps_genre) ILIKE '%music%' OR CONCAT(as_genre, ps_genre) ILIKE '%simulation%' OR CONCAT(as_genre, ps_genre) ILIKE '%play%' OR CONCAT(as_genre, ps_genre) ILIKE '%sports%' THEN 'games_entertainment'
						 					...
						 					WHEN CONCAT(as_genre, ps_genre) ILIKE '%navigation%' THEN 'navigation'
...
FROM both_stores
WHERE total_reviews > 300000 AND overall_rating > 4.0
